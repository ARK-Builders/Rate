...
1. Update `versionCode` and `versionName` in `app/build.gradle.kts`.
2. Put the production `app/google-services.json` from the secret storage.
3. Restore bundled fallback rates and icons as described in [develop.md](develop.md#step-2-hydrate-bundled-fallback-data).
4. Publish the GitHub APK release as described above and use it as a pre-Play-Store verification step.
...
6. Prepare release notes as described in [publication.md](publication.md).
7. Upload the generated Play Store artifact through Google Play Console and include the release notes.
8. Finish listing updates, website updates, and announcements as described in [publication.md](publication.md).
